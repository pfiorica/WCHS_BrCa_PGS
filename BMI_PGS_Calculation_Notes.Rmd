...
*Initial Point* (10 January 2026): The coverage of the variants above is far from ideal.  The original manuscript reads:  "SNPs with $MAF>.0.01$ in at least one population were kept in the analyses." That means that some of these variants could be somewhat rare in a given population ($MAF<0.05$) and kept in the analysis but near non-existent in another population. These likely would have been filtered out of our analysis with minor allele frequency filtering.

*Updated Point* (4 February 2026): The coverage issues above was resolved by editing the `UKB.PGS_PNF1_BMI.R` and `08_merge.valid.nvar_bmi_PNF.R` scripts. The issue was that variants with zero or NA effect sizes were being removed from the PGS calculation to decrease computing cost. However, after the fact, these variants were not removed from the `% Matched` calculation.
...
file_list <- list.files(weight_dir,pattern =  "(*.).txt")
...
From the information above, these PGS have much better coverage than those reported by Haiyang's code. Because of this, we will need to make some serious updates to his code...

## Identifying the problem

For some reason we are still running into very low calls on the PGS match rate. PGS003844, PGS003846, and PGS003847 have the correct match rates as shown above. 	
PGS003848 and PGS003845 have exceptionally low match rates.

The reason for this may be that most of these variants have VERY low effect sizes. i.e. zero effect.

For example, let's take a look at the PGS003845 downloaded from the PGS catalog for hg37.
```
rsID    chr_name        chr_position    effect_allele   other_allele    effect_weight   hm_source       hm_rsID hm_chr  hm_pos  hm_inferOtherAllele
rs10000008      4       172776204       T       C       0.0     ENSEMBL rs10000008      4       171855053       
rs10000011      4       138223055       T       C       0.0     ENSEMBL rs10000011      4       137301901       
rs1000002       3       183635768       T       C       0.0     ENSEMBL rs1000002       3       183917980       
rs1000003       3       98342907        G       A       0.0     ENSEMBL rs1000003       3       98624063        
rs10000030      4       103374154       A       G       0.0     ENSEMBL rs10000030      4       102452997       
rs1000007       2       237752054       C       T       -0.000165605867044904   ENSEMBL rs1000007       2       236843411    
```
...
fwrite(PGS_matched, pgs.dir %&% "WCHS_BrCa_BMI_5_PGS_with_WCHS_IDs.csv", col.names = T, row.names = F, sep= ",", quote = F)
```
